...
# Note: change global configuration to RENDER=True                             #
# ---------------------------------------------------------------------------- #
...
#     for i, rate in enumerate(query_rate_list):
...
#         # check number of rejected flows
#         n_reject = get_n_reject_avg(log_folder)
#         if n_reject > 0:
#             with counter.get_lock():
#                 counter.value += len(query_rate_list) - i - 1
#             return


#     # then we run until we have rejected flows
#     while n_reject < 1 and rate <= max_lambda_rate:
#         rate += 0.05
#         log_folder = os.path.join(foldername, 'rate{:.3f}'.format(rate))
#         Path(log_folder).mkdir(parents=False, exist_ok=True)
#         cmd = cmd_preamable + \
...
#     'oracle',  # 
#     'oracle-gsq',  # 
#     'kf1dlsq-ada',  # kf1d + lsq w/ adaptive sensor error
# ]
...
#     experiment_name = 'change-server-capacity'
#     root_dir = '../../data/simulation/'
#     data_dir = root_dir+experiment_name

#     for n_lb in n_lbs:
#         for n_as in n_ass:
#             if n_lb == 8 and n_as == 32: continue # skip this setup
...
#                 cmd_preamable = 'python3 run.py -t {} --n-lb {} --n-as {} --n-worker {} --cpu-fct-mu {} --process-n-stage {} --io-fct-mu {} --n-episode {} --first-episode-id {}'.format(
#                     t_episode, n_lb, n_as, n_worker, fct_mu, n_process_stage, fct_io, n_episode, first_episode_id)
