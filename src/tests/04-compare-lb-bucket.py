...
# query_rate_list = [r for r in query_rate_list if r <= 0.81]
...
n_episode = 5
fct_io = 0.25
n_lb_buckets = [1024, 2048, 4096, 8192, 16384, 32768, 65536]
...
if __name__ == "__main__":  # confirms that the code is under main function
    ...
    experiment_name = 'compare-lb-bucket-reduce'
        ...
        for n_as in n_ass:
            for n_worker in n_workers:
                for fct_mu in fct_mus:
                    ...
                    print(setup)
                    cmd_preamable = 'python3 run.py --n-lb {} --n-as {} --n-worker {} --cpu-fct-mu {} --process-n-stage {} --io-fct-mu {} --n-flow {} --n-episode {} --first-episode-id {}'.format(
                        n_lb, n_as, n_worker, fct_mu, n_process_stage, fct_io, n_flow_total, n_episode, first_episode_id)
                    for method in methods:
                        cmd = cmd_preamable + ' -m {}'.format(method)
                        if 'lsq' in method or 'sed' in method or 'hlb' in method:
                            for bucket_size in n_lb_buckets:
                                method_alias = '{}-{}bucket'.format(method, bucket_size)
                                cmd += ' --lb-bucket-size {}'.format(bucket_size)
