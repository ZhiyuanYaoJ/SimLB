...
def get_n_reject_avg(folder_name):
    ...
    n_reject = [int(l.split(' ')[-1]) for l in lines if 'number of rejected flow in total:' in l]
...
def work_log(work_data):
    ...
    for rate in query_rate_list:
        ...
        cmd = cmd_preamable + ' --lambda {0:.3f} -w {1} > {1}/test.log'.format(rate, log_folder)
        t0 = time.time()
...
n_process_stage = 1
max_lambda_rate = 1.5
fct_io = 0.2 # fixed average FCT of IO process
...
if __name__ == "__main__":  # confirms that the code is under main function
    ...
    experiment_name = 'test-b-reduce'
    root_dir = '../../data/simulation/'
    data_dir = root_dir+experiment_name

    for n_worker in n_workers:
        setup = setup_fmt.format(n_lb, n_as, n_worker, n_process_stage, fct_mu)
        if n_process_stage > 1:
            setup += '-{:.2f}iomu'.format(fct_io)
        cmd_preamable = 'python3 run.py --n-lb {} --n-as {} --n-worker {} --cpu-fct-mu {} --process-n-stage {} --io-fct-mu {} --n-flow {} --kf-sensor-std {}'.format(n_lb, n_as, n_worker, fct_mu, n_process_stage, fct_io, n_flow_total, kf_sensor_std)
        for method in methods:
            if '-dev' in method:
                for b_offset in b_offsets:
                    method_alias = method + '-b{:1d}'.format(b_offset)
                    log_folder = '/'.join([data_dir, setup, method_alias])
                    cmd = cmd_preamable + ' -m {} --b-offset {}'.format(method, b_offset)
