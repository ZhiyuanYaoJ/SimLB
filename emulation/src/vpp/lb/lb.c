...
next:
  ...
  vec_foreach(ip, to_be_updated) {
    ...
    // clib_warning("@lbshm: init as_index %u", shm_get_as_stat(&vip->shm, *ip)->as_index);
#endif // LB_STATS
  }
  vec_free(to_be_updated);
  ...
  vec_foreach(ip, to_be_added) {
    ...
    *as_index = as - lbm->ass;
#ifdef LB_STATS
    // clib_warning("@lbshm: SHM_N_BIN = %u b_header = before clear cache - 0x%016x = %llu\n", SHM_N_BIN, vip->shm.msg_out_cache->b_header, vip->shm.msg_out_cache->b_header);
    shm_as_clear_cache(&vip->shm, *as_index);
    // clib_warning("@lbshm: b_header = before - 0x%016x\n", vip->shm.msg_out_cache->b_header);
    SetBit (vip->shm.msg_out_cache->b_header, *as_index); /* make cell set */
    // clib_warning("@lbshm: after - 0x%016x ~ actual 0x%016x\n", vip->shm.msg_out_cache->b_header | ((u64)0x01 << (SHM_N_BIN - *as_index - 1)), vip->shm.msg_out_cache->b_header);
    shm_get_as_stat(&vip->shm, *as_index)->as_index = *as_index;
    // clib_warning("@lbshm: ip %u, as_index %llu, vip_index %u, vip->shm.name %s, &vip->shm.mem @%p, b_header @%p (- shm->n_as @%p = relative %u) 0x%08x, b_header=0x%x", 
    //   *ip, 
    //   *as_index, 
    //   vip_index, 
    //   vip->shm.name, 
    //   vip->shm.mem, 
    //   &(vip->shm.msg_out_cache->b_header),
    //   vip->shm.n_as, (char *) &(vip->shm.msg_out_cache->b_header) - (char *) vip->shm.n_as, vip->shm.msg_out_cache->b_header,
    //   vip->shm.msg_out_cache->b_header);
#endif /* LB_STATS */
...
#ifdef LB_STATS
      clib_warning("@lbshm: after - 0x%08x ~ actual 0x%08x\n", vip->shm.msg_out_cache->b_header | (0x01 << (SHM_N_BIN - *as_index - 1)), vip->shm.msg_out_cache->b_header);
      clib_warning("@lbshm: ----------");
#endif /* LB_STATS */
  }
  vec_free(to_be_added);
