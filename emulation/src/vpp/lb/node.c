      ...
      while (n_left_from > 0 && n_left_to_next > 0)
        {
...
#ifdef LB_SED
          u32 *as_iterator;
          f32 score0 = (vip_shm->msg_out_cache->body[asindex0].n_flow_on + 1)/(1e-9 + vip_shm->msg_in_cache->score[asindex0]), score1 = 0;
          ...
          // clib_warning("@lb:node sed asindex0 %d score0 %.4f score1 %.4f", asindex0, score0, score1);

#endif // LB_SED

#ifdef LB_SED2
          /* power of two */
          u32 asindex1 = vip0->new_flow_table[(hash0 + 1) & (vip0->new_flow_table_mask)].as_index;
          f32 score0 = (vip_shm->msg_out_cache->body[asindex0].n_flow_on + 1) / (1e-9 + vip_shm->msg_in_cache->score[asindex0]);
          f32 score1 = (vip_shm->msg_out_cache->body[asindex1].n_flow_on + 1) / (1e-9 + vip_shm->msg_in_cache->score[asindex1]);
          if (score1 < score0)
            asindex0 = asindex1;
            // clib_warning("@lb:node asindex0 %d score0 %.4f asindex1 %d score1 %.4f", asindex0, score0, asindex1, score1);
#endif // LB_SED2

#ifdef LB_LSQ // making load balancing decision based on instantaneous number of established flows
              u32 *as_iterator;
              f32 score0 = vip_shm->msg_out_cache->body[asindex0].n_flow_on, score1;
              vec_foreach(as_iterator, vip0->as_indexes)
              {
                score1 = vip_shm->msg_out_cache->body[*as_iterator].n_flow_on;
                if (score1 < score0)
                {
                  asindex0 = *as_iterator;
                  score0 = score1;
              ...
              /* power of two */
              u32 asindex1 = vip0->new_flow_table[(hash0 + 1) & (vip0->new_flow_table_mask)].as_index;
              f32 score0 = vip_shm->msg_out_cache->body[asindex0].n_flow_on;
              f32 score1 = vip_shm->msg_out_cache->body[asindex1].n_flow_on;
              if (score1 < score0)
                asindex0 = asindex1;
              // clib_warning("@lb:node asindex0 %d score0 %.4f asindex1 %d score1 %.4f", asindex0, score0, asindex1, score1);
#endif // LB_LSQ2
              ...
              // clib_warning("@lb:node rand_num %.6f bucket %d (asindexes[bucket]=%d) odd %.6f alias %d (asindexes[alias]=%d) chosen one %d", rand_num, bucket, vip0->as_indexes[bucket], odd, alias, vip0->as_indexes[alias], asindex0);
...
#ifdef LB_NF_CO
          if (asindex0 > 24 - LB_NF_CO) vip_shm->msg_in_cache->score[asindex0] += 0.66 * (float) sb->d_n_flow; // double the metric since the latter four AS has limited processing capability
#endif // LB_NF_CO
